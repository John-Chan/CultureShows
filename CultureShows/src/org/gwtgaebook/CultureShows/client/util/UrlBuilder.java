package org.gwtgaebook.CultureShows.client.util;
...
public class UrlBuilder {
	...
	private void setDefaultValues(String url) {
		if (url == null || url.length() < URL_MIN_LENGTH) {
			throw new IllegalArgumentException(
					"Cannot get default values from \"" + url
							+ "\": URL is to short.");
		}
		...
		try {
			...
			String hostAndContext = url.substring(index
					+ PROTOCOL_HOSTNAME_SEPARATOR.length());
	...
	public final String toUrl() {
		assertValid(context,
				"No context given. Please call setContext() first.");
		if (resourcePaths.isEmpty()) {
			throw new IllegalStateException(
					"No resource paths given. Please call addResourcePath(String...) first.");
		}
	...
	public UrlBuilder setHost(String host) {
		...
		if (host.contains(":")) {
			...
			if (parts.length > 2) {
				throw new IllegalArgumentException(
						"Host contains more than one ':': " + host);
			}
			try {
				setPort(Integer.parseInt(parts[1]));
			} catch (NumberFormatException e) {
				throw new IllegalArgumentException(
						"Port is not a valid number: " + host);
			}
			host = parts[0];
		}
		assertValid(host, "Host must not be empty");
	...
	public UrlBuilder setPort(int port) {
		if (port < MIN_PORT || port > MAX_PORT) {
			throw new IllegalArgumentException("Port is not in range ["
					+ MIN_PORT + ", " + MAX_PORT + "].");
		}
		this.port = port;
		return this;
	...
	private void assertValid(String value, String message)
			throws IllegalArgumentException {
	...
	private void assertNotEmpty(Object[] values, String message)
			throws IllegalArgumentException {
		if (values == null) {
			throw new IllegalArgumentException(message);
		}
