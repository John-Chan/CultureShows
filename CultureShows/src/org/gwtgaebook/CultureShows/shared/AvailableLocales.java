...
/**
 * <b>Important!</b> This file has to be kept up to date and translated whenever
...
public class AvailableLocales {
	...
	public static class LocaleInfo {
		 ...
		 * 
		 * @param locale
		 *            The locale identifier, an IETF language tag.
		 * @param name
		 *            The name of the locale, in the locale's language.
		 * @param switchMessage
		 *            A message prompting to switch to that locale, in that
		 *            locale's language.
		 */
		...
		/**
		 * @return A message prompting to switch to that locale, in that
		 *         locale's language.
		 */
	...
	public AvailableLocales() {
		...
		localeInfos.add(new LocaleInfo("fr", "fran\u00e7ais",
				"Passer au fran\u00e7ais?"));
	}

	/**
	 * Finds the best matching locale in the list of locales. Given the
	 * two-letter locale 'en', the algorithm will prefer: 1) 'en' 2) 'en-US',
	 * 'en-CA' 3) The first locale in the list Given the four-letter local
	 * 'en-US', the algorithm will prefer: 1) 'en-US' 2) 'en' 3) 'en-CA' 4) The
	 * first locale in the list If more than one available locale matches at the
	 * same preference level, the first one encountered will be returned.
	 * 
	 * @param locale
	 *            The desired locale.
	...
	public LocaleInfo getBestLocale(final String locale) {
		...
		for (LocaleInfo info : localeInfos) {
			...
			if (preferenceLevel <= 2) {
				// Preference is the smallest possible, only a perfect match can
				// win.
				continue;
			}
			...
			if (shortLocale != null) {
				...
				if (infoLocale.equalsIgnoreCase(shortLocale)) {
					...
					preferenceLevel = 2;
						...
						&& infoLocale.substring(0, 2).equalsIgnoreCase(
								shortLocale)) {
					result = info;
					preferenceLevel = 3;
				}
	 ...
	 * 
	 * @param index
	 *            The index of the locale to access
	 ...
	 * 
	 * @param locale
	 *            The locale to look for, an IETF language tag.
	...
	/**
	 * @return The index of the default locale, always 0 since the first locale
	 *         is the default one.
	 */
